...
class Backend:
    ...
    def __init__(self, api_key: Optional[str] = None, headers: Optional[dict] = None):
        ...
        self.session.headers.update({
            ...
            'User-Agent': f'tpuf-python/{tpuf.VERSION} {user_agent}'
        })
    ...
    def __eq__(self, other):
        if isinstance(other, Backend):
            return (
                ...
                self.api_base_url == other.api_base_url and 
    ...
    def make_api_request(self,
        ...
        request = requests.Request(
            method or 'GET',
            self.api_base_url + '/v1/' + '/'.join(args)
        )
            ...
            try:
                ...
                if len(server_timing_str) > 0:
                    ...
                    match_processing = re.match(
                        r'.*processing_time;dur=([\d\.]+)',
                        server_timing_str
                    )
                    if match_processing:
                        try:
                            performance['server_time'] = float(
                                match_processing.group(1)
                            ) / 1000.0
                        except ValueError:
                            pass
    ...
    async def make_api_request_async(self,
        ...
        timeout = aiohttp.ClientTimeout(
            connect=tpuf.connect_timeout,
            total=tpuf.connect_timeout + tpuf.read_timeout
        )
